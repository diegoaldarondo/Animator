...
h{2} = Keypoint2DAnimator(markers, skeleton,'Position',[0 0 1 1]);
axis off;
...
h{4} = Keypoint2DAnimator(markers, skeleton,'Position', [.75 .75 .25 .25]);
axis off;
Animator.linkAll(h)
set(fMulti,'pos',pos);
...
h = cell(3,1);
h{1} = VideoAnimator(V,'Position',[0 0 1 1]);
% Specify position relative to an axis
h{2} = IndicatorAnimator(logic, 'Axes', h{1}.Axes, 'patchX', [0 .25 .25 0]*size(V,1), 'patchY', [0 0 .25 .25]*size(V,2));
% Or to the figure
...
h{2} = ScatterAnimator([X; Y]', 'id', 1);
h{3} = ScatterAnimator([Y; X]', 'id', 2);
Animator.linkAll(h)
Animator.tileAnimators(h);

% Access the axes using the Axes property as below:
cellfun(@(X) set(X.Axes, 'color', 'k', 'XTick', [], 'YTick', []), h)
...
h = cell(1);
h{1} = QuiverAnimator([X; Y]', 'step', .1, 'vectorSize', .1,...
                      'densityThresh', 1, 'cmap', @parula,'smooth',true,'id',1);
...
% Unfortunately, we need to remove the x tick labels until I write an auto
% tick mode that plays nice with animation. 
set(h{1}.Axes, 'XTick',[])
...
cellfun(@(X) set(X.Axes,'color','k'), h)
Animator.linkAll(h)
